 ...
 */
	...
	function beforeFilter() { 
        parent::beforeFilter(); 
        // CSRF Protection
    	if (in_array($this->params['action'], array('add', 'edit','reset_password','edit_teacher','edit_student'))) {
 ...
 */
	public function admin_index() {
		...
		// 	'conditions' => array('User.role_id !=' => array(4,5))
		// );
 ...
 */
	...
	public function reset_password($id = null) {
		...
		if (!empty($this->request->data)) {
			if ($this->User->save($this->request->data)) {
				$this->Session->setFlash(__d('croogo', 'Mật khẩu đã được thay đổi.'), 'flash', array('class' => 'btn-warning'));
				return $this->redirect(array('action' => 'index'));
			} else {
				$this->Session->setFlash(__d('croogo', 'Password could not be reset. Please, try again.'), 'flash', array('class' => 'error'));
			}
 ...
 */
	public function index() {
		$this->set('title_for_layout', __d('croogo', 'Tài khoản của tôi'));
	}
	...
	public function teachers(){
		...
		$teachers = $this->User->find('all', array(
			...
			'fields' => array('User.name','User.username','Department.id','Department.name','Subject.name','User.university')
		));
		...
		$this->set(compact('teachers','total'));
	}
	...
	public function managers(){
		$this->set('title_for_layout', __d('croogo', 'Ban Giám Hiệu'));
		$this->User->recursive = 0;
		$managers = $this->User->find('all', array(
			'conditions' => array('User.department_id' => 10),
			'fields' => array('User.id','User.name','User.username','Department.name','Subject.name','User.position','User.university'),
			'order' => array('User.created' => 'ASC')
		));
		$this->set(compact('managers'));
	}
	//Danh sách học sinh front end
	public function students(){
		...
		$students = $this->User->find('all', array(
			...
			'order' => array('User.classroom_id' => 'ASC')
		));
		$classrooms = $this->User->Classroom->find('all', array(
			'fields' => array('Classroom.id','Classroom.name')
		));
		$years = $this->User->Year->find('all', array(
			'fields' => array('Year.id','Year.name')
		));
		$this->set(compact('students','classrooms','years'));
	}
	// Danh sách học sinh back end
	...
	//Điểm thi học sinh
	public function point($slug=null, $classid=null){
		$this->layout = 'points';
		$this->loadModel('Subject');
		if (!$this->User->exists($slug) && !$this->User->exists($classid)) {
			throw new NotFoundException(__('Không tồn tại điểm thi'));
		}
		$students = $this->User->find('first', array(
			'conditions' => array(
				'User.username' => $slug,
			)
		));
		$classid = explode('-', $classid);
		$points = $this->User->Point->find('first', array(
			'conditions' => array(
				'Point.classroom_id' => $classid[0]
			)
		));
		//pr($points); die();
		$options = array(
			'conditions' => array(
				'User.username' => $slug,
				'User.classroom_id LIKE' =>  '%"'.$classid[0].'"%',
				//'Point.classroom_id' => $classid[0]
			),
		);
		//pr($classid[0]);
		...
		$this->set(compact('students','subjects','classid','points'));
		$this->set('title_for_layout', __d('croogo','Điểm thi của '.$students['User']['name'].''));
	}
	...
	public function years(){
		...
		if($yearid!=0){
			...
			$this->set(compact('students','total','classrooms','years'));
		}else{
			$total = $this->User->find('count', array(
				'conditions' => array('User.role_id' => 5)
			));
			...
			$this->set(compact('total','classrooms','years','students'));
		}
		$this->render('years');
	}
	...
	public function classrooms(){
		...
		if($classid!=0){
			$students = $this->User->find('all', array(
				'conditions' => array(
					'User.classroom_id LIKE' => '%"'.$classid.'"%',
					'User.role_id' => 5
				)
			));
			$total = $this->User->find('count', array(
 ...
 */
	public function add() {
		$this->set('title_for_layout', __d('croogo', 'Đăng ký tài khoản'));
		if (!empty($this->request->data)) {
			$this->User->create();
			...
			if ($this->User->save($this->request->data)) {
				...
				$this->Session->setFlash(__d('croogo', 'Đăng ký thành công!'), 'flash', array('class' => 'success'));
		...
		$roles = $this->User->Role->find('list', array(
			'conditions' => array('Role.id' => array(4,5,6))
		));
		$this->set(compact('roles'));
	}
 ...
 */
	public function edit($id = null) {
		if (!empty($this->request->data)) {
			if ($this->User->saveAssociated($this->request->data)) {
				$this->Session->setFlash(__d('croogo', 'Thông tin đã được cập nhật'), 'flash', array('class' => 'alert alert-success'));
				return $this->Croogo->redirect(array('plugin' => 'users','controller' => 'users','action' => 'edit', 'id' => $this->User->id));
			} else {
				$this->Session->setFlash(__d('croogo', 'The User could not be saved. Please, try again.'), 'flash', array('class' => 'alert alert-warning'));
			}
		} else {
			$this->request->data = $this->User->read(null, $id);
		}
		...
		$this->set(compact('roles'));
		$this->set('editFields', $this->User->editFields());
		$this->set('title_for_layout', __d('croogo', 'Cập nhật tài khoản'));
	}
	//Cập nhật thông tin giáo viên
	public function edit_teacher($id = null) {
		if (!empty($this->request->data)) {
			if ($this->User->saveAssociated($this->request->data)) {
				$this->Session->setFlash(__d('croogo', 'Thông tin giáo viên đã được cập nhật'), 'flash', array('class' => 'alert alert-success'));
				return $this->Croogo->redirect(array('plugin' => 'users','controller' => 'users','action' => 'edit_teacher', 'id' => $this->User->id));
			} else {
				$this->Session->setFlash(__d('croogo', 'The User could not be saved. Please, try again.'), 'flash', array('class' => 'error'));
			}
		} else {
		...
		$roles = $this->User->Role->find('list');
		$subjects = $this->User->Subject->find('list', array(
			'fields' => array('Subject.id', 'Subject.name'),
			'order' => array('Subject.name')
		));
		...
		$this->set(compact('roles','subjects','departments','name'));
		$this->set('editFields', $this->User->editFields());
	...
	public function edit_student($id = null) {
		if (!empty($this->request->data)) {
			$this->request->data['User']['year_id'] = '["'.$this->request->data['User']['year_id1'].'","'.$this->request->data['User']['year_id2'].'","'.$this->request->data['User']['year_id3'].'"]';
			$this->request->data['User']['classroom_id'] = '["'.$this->request->data['User']['classroom_id1'].'","'.$this->request->data['User']['classroom_id2'].'","'.$this->request->data['User']['classroom_id3'].'"]';
			//pr($this->request->data['User']['year_id']).'<br>';
			//pr($this->request->data['User']['classroom_id']);die();
			if ($this->User->saveAssociated($this->request->data)) {
				$this->Session->setFlash(__d('croogo', 'Thông tin học sinh đã được cập nhật'), 'flash', array('class' => 'alert alert-success'));
				return $this->Croogo->redirect(array('plugin' => 'users','controller' => 'users','action' => 'edit_student', 'id' => $this->User->id));
			} else {
				$this->Session->setFlash(__d('croogo', 'The User could not be saved. Please, try again.'), 'flash', array('class' => 'error'));
			}
		} else {
		...
		$years = $this->User->Year->find('list');
		$classrooms = $this->User->Classroom->find('list');
		$this->set(compact('roles','name','years','classrooms'));
		$this->set('editFields', $this->User->editFields());
		$this->set('title_for_layout', __d('croogo', 'Cập nhật thông tin học sinh '.$name['User']['name']));
	}
 ...
 */
	public function forgot() {
		$this->set('title_for_layout', __d('croogo', 'Quên mật khẩu'));
	...
	public function reset($username = null, $key = null) {
		...
		if (!empty($this->request->data) && isset($this->request->data['User']['password'])) {
			...
			if ($this->User->save($user['User'], $options)) {
				$this->Session->setFlash(__d('croogo', 'Your password has been reset successfully.'), 'flash', array('class' => 'success'));
				return $this->redirect(array('action' => 'login'));
			} else {
				$this->Session->setFlash(__d('croogo', 'An error occurred. Please try again.'), 'flash', array('class' => 'error'));
			}
 ...
 */
	public function login() {
		$this->set('title_for_layout', __d('croogo', 'Đăng nhập'));
		if ($this->request->is('post')) {
 ...
 */
	public function logout() {
		...
		$this->Session->setFlash(__d('croogo', 'Đăng xuất thành công.'), 'flash', array('class' => 'success'));
 ...
 */
	public function view($username = null) {
		...
		$this->set(compact('user','classrooms'));
		$this->set('title_for_layout', __d('croogo', 'Xem thông tin '.$user['Role']['title'].' '.$user['User']['name'].''));
	}
