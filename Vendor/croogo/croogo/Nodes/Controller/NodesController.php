 ...
 */	public function thongbao() {
		...
		$this->set('title_for_layout', __d('croogo', 'Thông báo'));
	}
	public function lich_cong_tac() {
		$this->Node->recursive = 0;
		$nodes = $this->Node->find('all');
		$this->set(compact('nodes'));
		$this->set('title_for_layout', __d('croogo', 'Lịch công tác'));
	}
	//Calendar
	public function calendar(){
    	Configure::write('debug', 0);
		$this->autoLayout = false;
		$nodes = $this->Node->find('all',array('order' => 'Node.created DESC','conditions'=>array('type'=>array('thong-bao'))));
		$json = array();
		foreach($nodes as $n){
			$json[] = array(
				'id'=>$n['Node']['id'],
				'title'=>$n['Node']['title'],
				'start'=>$n['Event']['start_date'],
				'bd'=>$n['Event']['start_date'],
				'stop_'=>$n['Event']['end_date'],
				'end'=> $n['Event']['start_date'],
				'created'=>$n['Node']['created'],
				'des'=>$n['Node']['body'],
				'url' =>  Router::url(array('plugin'=>'nodes','controller'=>'nodes', 'action'=>'view', 'slug' => $n['Node']['slug'], 'type' => 'thong-bao')),
				'color' => '#d9534f',
			);
		}
    ...
    public function congtac(){
    	Configure::write('debug', 0);
		$this->autoLayout = false;
		$nodes = $this->Node->find('all',array('order' => 'Node.created DESC','conditions'=>array('type'=>array('lich-cong-tac'))));
		$json = array();
		foreach($nodes as $n){
			$json[] = array(
				'id'=>$n['Node']['id'],
				...
				'created'=>$n['Node']['created'],
				'des'=>$n['Node']['body'],
				'url' =>  Router::url(array('plugin'=>'nodes','controller'=>'nodes', 'action'=>'view', 'slug' => $n['Node']['slug'], 'type' => 'lich-cong-tac')),
				'color' => '#0199cb',
			);
		}
		$this->set('json', json_encode($json));
 ...
 */	
	...
	public function home(){
		$this->set('title_for_layout', __d('croogo', 'Hệ thống quản lý trường học'));
		$sliders = $this->Node->find('all', array(
			'conditions' => array('Node.type' => 'slider')
		));
		if (!empty($this->request->params['requested'])) {
			return $sliders;
		}
		$term = 'tin-hoat-dong';
		$news = $this->Node->find('all', array(
			'conditions' => array(
				'Node.type' => 'tin-tuc',
				'Node.terms LIKE' =>  '%"'.$term.'"%'
			),
			'limit' => 6
		));
		$terms = 'cau-lac-bo';
		$clubs = $this->Node->find('all', array(
			'conditions' => array(
				'Node.type' => 'tin-tuc',
				'Node.terms LIKE' =>  '%"'.$terms.'"%'
			),
			'limit' => 6
		));
		if (!empty($this->request->params['requested'])) {
		...
		$notices = $this->Node->find('all', array(
			'conditions' => array('Node.type' => 'thong-bao'),
			'limit' => 6
		));
		$calendars = $this->Node->find('all', array(
			'conditions' => array('Node.type' => 'lich-cong-tac'),
			'limit' => 6
		));
		$this->set(compact('sliders','news','notices','calendars'));
	}
	// Gallery
